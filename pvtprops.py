...
# Functions for calculating critical pressure and temperature
...
# Functions for Carr-Kobayashi correction
...
# Functions for Wichert-Aziz correction
...
def wichert_aziz_pressure(sg_gas, CO2, H2S):
    ...
    B = H2S
    epsilon = 120 * (A ** 0.9 - A ** 1.6) + 15 * (B ** 0.5 - B ** 4)
    pressure_corrected = (critical_presssure_Sutton(sg_gas) * wichert_aziz_temperature(sg_gas, CO2, H2S)) / \
                         (critical_temperature_Sutton(sg_gas) + H2S * (1 - H2S) * epsilon)
    return pressure_corrected
...
def redlich_kwong_eos(P, T, sg_gas, CO2, H2S, method, N2):  # T in Fahrenheit
    ...
    if method == "WichertAziz":
        a = (0.42727 * (R ** 2) * (wichert_aziz_temperature(sg_gas, CO2, H2S) ** 2.5)) / \
            (wichert_aziz_pressure(sg_gas, CO2, H2S))
        b = 0.08664 * (R * wichert_aziz_temperature(sg_gas, CO2, H2S)) / \
            (wichert_aziz_pressure(sg_gas, CO2, H2S))
